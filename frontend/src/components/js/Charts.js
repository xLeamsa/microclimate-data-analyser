...
// Centralne zarządzanie kolorami linii wykresów
const COLORS = {
    temperature: '#e63946',
    humidity: '#457b9d',
    co2: '#2a9d8f',
...
function Charts() {
    ...
    const fetchHistory = async () => {
        try {
            const response = await axios.get(`http://127.0.0.1:5000/api/measurements/history?range=${timeRange}`);
            const formattedData = response.data.map(item => ({
                ...item,
                displayTime: new Date(item.timestamp).toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit' })
            }));
                        ...
                        <div className="chart-container-small">
                            <ResponsiveContainer>
                                <AreaChart data={historyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                    ...
                                    <YAxis stroke={COLORS.temperature} domain={['auto', 'auto']} />
                                    <Tooltip />
